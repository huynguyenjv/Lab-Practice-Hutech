...
public class CategoryController {
    ...
    public String deleteCategory(@PathVariable("id") String id, Model model) {
        Category category = categoryService.getCategoryById(id)
                .orElseThrow(() -> new IllegalArgumentException("Invalid category Id:" + id));
        if(!categoryService.checkCategoryHasProduct(id)){
            System.out.println(categoryService.checkCategoryHasProduct(id));
            model.addAttribute("error", "Không thể xóa danh mục đã có sản phẩm!");
            model.addAttribute("categories", categoryService.getAllCategories());
            return "/categories/list-category";
        }else{
